...
export function SiteHeader() {
  return (
    <header className="sticky top-0 z-40 border-b border-line bg-surface/95 backdrop-blur">
      <div className="mx-auto flex min-h-16 max-w-7xl items-center justify-between gap-3 px-5 sm:px-8 lg:px-10">
        <Link href="/" className="flex min-w-0 items-center gap-3 py-3">
          <BrandMark className="h-10 w-10 shrink-0 text-foreground sm:h-9 sm:w-9" />
          <span className="min-w-0">
            <span className="block truncate text-sm font-semibold text-foreground">{profile.name}</span>
            <span className="hidden truncate text-xs text-muted sm:block">{profile.role}</span>
          </span>
        </Link>
        ...
        <div className="flex shrink-0 items-center gap-2">
          <MobileCallButton />
          <Link href="/resume" className="on-dark hidden min-h-11 items-center gap-2 rounded-md bg-foreground px-3 text-sm font-semibold transition-colors hover:bg-accent sm:inline-flex sm:px-4">
