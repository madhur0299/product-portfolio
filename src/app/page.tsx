...
const headlineResults = [
  { value: "+10%", label: "Booking conversion", context: "PRISM" },
  ...
  return (
    <>
      ...
      <main id="content" className="bg-background text-foreground">
        <section className="relative overflow-hidden bg-foreground text-on-dark">
          ...
          <div className="relative mx-auto flex min-h-[32rem] max-w-[90rem] flex-col justify-center px-5 py-8 md:min-h-[30rem] md:px-10 md:py-6 xl:px-16">
            <div className="max-w-5xl">
              <h1 className="max-w-[16ch] text-[2.7rem] font-semibold leading-[1.01] text-balance sm:text-6xl md:max-w-[21ch] md:text-6xl lg:text-7xl xl:text-[4.75rem]">
                I turn consumer growth problems into <span className="text-white/50">measurable outcomes.</span>
              </h1>
              <p className="mt-5 max-w-3xl text-[0.95rem] leading-6 text-white/75 md:hidden">At PRISM, I lead pricing and conversion for OYO US and G6, while building AI systems that reduce operating effort and improve decisions.</p>
              <p className="mt-7 hidden max-w-3xl text-lg leading-8 text-white/75 md:block">{profile.summary}</p>
              <div className="mt-7 flex flex-col gap-3 sm:mt-9 sm:flex-row">
                <Link href="/experience" className="on-light inline-flex min-h-12 items-center justify-center gap-3 rounded-full bg-surface px-6 text-sm font-semibold transition-colors hover:bg-surface-muted">Explore experience <ArrowRight className="h-4 w-4" aria-hidden="true" /></Link>
                <a href={`mailto:${profile.email}?subject=Product%20conversation`} className="hidden min-h-12 items-center justify-center gap-3 rounded-full border border-white/35 px-6 text-sm font-semibold transition-colors hover:bg-white/10 sm:inline-flex">Start a conversation <Mail className="h-4 w-4" aria-hidden="true" /></a>
          ...
          <dl className="relative mx-auto grid max-w-[90rem] grid-cols-3 border-t border-white/15 bg-white/[0.05] px-5 md:px-10 xl:px-16">
            {headlineResults.map((result, index) => (
              <div key={result.label} className={`py-5 md:py-8 ${index > 0 ? "border-l border-white/15 pl-4 md:pl-8" : "pr-4"}`}>
                <dt className="font-mono text-[0.58rem] uppercase text-white/55 md:text-[0.68rem]">{result.context}</dt>
        ...
        <section className="mx-auto max-w-[90rem] px-5 py-16 md:px-10 md:py-28 xl:px-16">
          <div className="grid gap-8 border-b border-line pb-10 md:grid-cols-[minmax(0,0.8fr)_minmax(20rem,0.55fr)] md:items-end">
            <h2 className="max-w-3xl text-4xl font-medium leading-tight text-balance md:text-6xl">A career across three different product systems.</h2>
            <p className="max-w-xl text-base leading-7 text-muted">From D2C ecommerce to subscription learning and US hospitality, each role expanded the product problem, business model, and operating complexity I owned.</p>
          </div>
          <div className="divide-y divide-line">
            {professionalExperience.map((experience) => (
              <Link key={experience.company} href="/experience" className="group grid gap-5 py-8 transition-colors hover:bg-surface-muted md:grid-cols-[minmax(13rem,0.45fr)_minmax(0,1fr)_auto] md:items-center md:px-4">
        ...
        <section className="bg-foreground text-on-dark">
          <div className="mx-auto grid max-w-[90rem] gap-12 px-5 py-16 md:px-10 md:py-24 lg:grid-cols-[minmax(0,0.75fr)_minmax(22rem,0.55fr)] lg:items-center xl:px-16">
            <div>
              <h2 className="max-w-3xl text-4xl font-medium leading-tight text-balance md:text-6xl">Coach is where I test product judgment by building.</h2>
        ...
        <section className="mx-auto max-w-[90rem] px-5 py-16 md:px-10 md:py-28 xl:px-16">
          <div className="grid gap-8 md:grid-cols-[minmax(0,0.75fr)_minmax(20rem,0.5fr)] md:items-end">
            <h2 className="text-4xl font-medium leading-tight md:text-6xl">Capabilities grounded in shipped work.</h2>
